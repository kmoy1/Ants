...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        """
        if insect.is_ant: #If the passed in insect is an ant
            if not self.ant: #IF no ant exists at place. 
                self.ant = insect
            elif self.ant: #Ant exists in place. 
                #CASE 1: Current place holds bodyguard ant, passing in normal ant
                if self.ant.is_container and self.ant.can_contain(insect): 
                    self.ant.contain_ant(insect) 
                #CASE 2: Current place holds normal ant, passing in bodyguard ant
                elif insect.is_container and insect.can_contain(self.ant): 
                    insect.contain_ant(self.ant)
                    self.ant = insect 
                #CASE 3: neither ant can contain the other
                else: 
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if current_place.ant:
            if current_place.ant.blocks_path:
                return True
            else:
                return False
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def check_next_place(current_place):
            nonlocal places_counted #increment places_counted in outer environment
            places_counted += 1
            #Terminate if thrower ant's search reached the hive OR past max range (shortthrower ants)
            if current_place == hive or places_counted > self.max_range:
                return None 
            elif current_place.bees: #if current place has bees
                if places_counted < self.min_range: #haven't checked past minimum range of ant.
                    return check_next_place(current_place.entrance) #check next place, no matter if there are bees in current place or not 
                else:
                    return random_or_none(current_place.bees)
            #No bees found in current place- check next
            else:
                return check_next_place(current_place.entrance)

        if self.place.bees:
            if self.min_range == 0:
                return random_or_none(self.place.bees)
        else:
            return check_next_place(self.place.entrance)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        death_bed = self.place
        Ant.reduce_armor(self, amount)
        if self.armor <= 0:
            bees_copy = death_bed.bees[:]            
...
class QueenAnt(ScubaThrower):
    ...
    true_queen = False
    implemented = True   # Change to True to view in the GUI
    doubled_ants = [] #List of ants w/ doubled damage
    # END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.true_queen:
            self.name = "Fake Queen" #doesn't happen for first queen creation.
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self.name == "Fake Queen":   
            self.reduce_armor(self.armor)
        else:
            current_exit = self.place.exit
            while current_exit: #place behind current exit exists
                if current_exit.ant and current_exit.ant not in QueenAnt.doubled_ants: #standard non-bodyguard ant
                    QueenAnt.doubled_ants.append(current_exit.ant)
                    current_exit.ant.damage *= 2
                if current_exit.ant and current_exit.ant.is_container:
                    if current_exit.ant.contained_ant and current_exit.ant.contained_ant not in QueenAnt.doubled_ants:
                        QueenAnt.doubled_ants.append(current_exit.ant.contained_ant)
                        current_exit.ant.contained_ant.damage *= 2
